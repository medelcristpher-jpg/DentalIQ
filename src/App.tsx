...
const SESSION_KEY = 'dentaiq_session_v5'
...
// ── SESIÓN ────────────────────────────────────────────────────────
const saveSession = u => { try{localStorage.setItem(SESSION_KEY,JSON.stringify({user:u,savedAt:Date.now()}))}catch{} }
const clearSession = () => { try{localStorage.removeItem(SESSION_KEY)}catch{} }
const loadSession = () => {
  try {
    const d=JSON.parse(localStorage.getItem(SESSION_KEY))
    if(!d) return null
    return {user:d.user, expired:(Date.now()-d.savedAt)/60000>=50}
  } catch{return null}
}
...
const loadGIS = () => new Promise(r=>{
  ...
  const s=document.createElement('script'); s.src='https://accounts.google.com/gsi/client'; s.onload=()=>r(); document.head.appendChild(s)
})
  ...
  return new Promise((resolve,reject)=>{
    const c=window.google.accounts.oauth2.initTokenClient({
      ...
      callback:async r=>{
        ...
        try{const u=await fetch('https://www.googleapis.com/oauth2/v2/userinfo',{headers:{Authorization:`Bearer ${r.access_token}`}}).then(x=>x.json());resolve({email:u.email,name:u.name||u.email,picture:u.picture||'',token:r.access_token})}catch(e){reject(e)}
      },
...
// ── DRIVE API ─────────────────────────────────────────────────────
const driveHeaders = t => ({Authorization:`Bearer ${t}`,'Content-Type':'application/json'})
...
async function driveCreate(t,body){return fetch('https://www.googleapis.com/drive/v3/files',{method:'POST',headers:driveHeaders(t),body:JSON.stringify(body)}).then(r=>r.json())}
...
async function driveReadCSV(t,fileId){
  // Exportar planilla como CSV
  const r=await fetch(`https://www.googleapis.com/drive/v3/files/${fileId}/export?mimeType=text/csv`,{headers:{Authorization:`Bearer ${t}`}})
  if(!r.ok) throw new Error(`HTTP ${r.status}`)
  return await r.text()
}

async function getOrCreate(t,name,parentId){
  const q=parentId?`name='${name}' and '${parentId}' in parents and mimeType='application/vnd.google-apps.folder' and trashed=false`:`name='${name}' and mimeType='application/vnd.google-apps.folder' and 'root' in parents and trashed=false`
...
async function createSheetIfNotExists(t,name,parentId,rows){
  ...
  try{await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${file.id}/values/A1?valueInputOption=USER_ENTERED`,{method:'PUT',headers:driveHeaders(t),body:JSON.stringify({values:rows,majorDimension:'ROWS'})})}catch{}
  return file.id
}

async function appendRowToSheet(token,fileId,values){
  try{await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${fileId}/values/A1:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`,{method:'POST',headers:driveHeaders(token),body:JSON.stringify({values:[values],majorDimension:'ROWS'})}); return true}catch{return false}
}
...
async function findOrCreateMonthSheet(token,folderId,tipo){
  ...
  const headers=tipo==='Ingresos'?['Fecha','Nombre Paciente','Servicio Realizado','Monto (CLP)','Método de Pago','Notas']:['Fecha','Descripción del Gasto','Categoría','Monto (CLP)','Proveedor / Empresa','Notas']
...
// ── SETUP DRIVE — solo dentro de DentaIQ ─────────────────────────
async function setupDrive(t){
  // Solo trabaja con la carpeta DentaIQ y sus subcarpetas
  const rootId=await getOrCreate(t,'DentaIQ')
  const now=new Date(), año=now.getFullYear(), mes=now.toLocaleDateString('es-CL',{month:'long'}), hoy=now.toLocaleDateString('es-CL')

  // Crear Ingresos y Gastos si no existen
  ...
  if(ingFiles.length===0) await createSheetIfNotExists(t,`Ingresos ${mes} ${año}`,ingId,[['Fecha','Nombre Paciente','Servicio Realizado','Monto (CLP)','Método de Pago','Notas'],[hoy,'Ejemplo Paciente','Limpieza dental','35000','Efectivo','Ejemplo']])
  const gasFiles=await driveListFiles(t,gasId)
  if(gasFiles.length===0) await createSheetIfNotExists(t,`Gastos ${mes} ${año}`,gasId,[['Fecha','Descripción del Gasto','Categoría','Monto (CLP)','Proveedor / Empresa','Notas'],[hoy,'Insumos dentales','Insumos','245000','3M Chile','Ejemplo']])

  // Descubrir TODAS las subcarpetas de DentaIQ (solo ahí, no en Drive raíz)
...
// ── PARSEAR CSV PARA IA — extrae resumen financiero ───────────────
function resumirCSVParaIA(csv, nombreArchivo) {
  const lines = csv.split('\n').map(l=>l.trim()).filter(l=>l)
  if(lines.length<2) return null

  const headers = lines[0].split(',').map(h=>h.toLowerCase().trim())

  // Detectar si es archivo de ingresos
  const esIngreso = nombreArchivo.toLowerCase().includes('ingreso') || headers.some(h=>h.includes('paciente')||h.includes('boleta')||h.includes('procedimiento'))
  const esGasto = nombreArchivo.toLowerCase().includes('gasto') || headers.some(h=>h.includes('proveedor')&&!h.includes('inventario'))

  // Buscar columnas de montos
  const montoIdxs = []
  for(let i=0;i<headers.length;i++){
    const h=headers[i]
    if(h.includes('monto')||h.includes('total')||h.includes('efectivo')||h.includes('transferencia')||h.includes('tarjeta')||h==='precio'||h.includes('cantidad a pagar')) montoIdxs.push(i)
  }
  ...
  for(let i=1;i<lines.length;i++){
    ...
    for(const idx of montoIdxs){
      const val = cols[idx]?.replace(/[$,\s]/g,'')
      const n = parseFloat(val)
      if(!isNaN(n)&&n>0){
        // Si parece USD (< 1000), convertir a CLP aproximado (1 USD ≈ 930 CLP)
        totalMonto += n < 1000 ? n*930 : n
        break // solo sumar una vez por fila
      }
    }

    // Contar procedimientos o proveedores
    if(esIngreso){
      const procIdx = headers.findIndex(h=>h.includes('procedimiento')||h.includes('servicio'))
      if(procIdx>=0&&cols[procIdx]){
        const p=cols[procIdx].slice(0,40)
        procedimientos[p]=(procedimientos[p]||0)+1
      }
    }
    if(esGasto){
      const provIdx=headers.findIndex(h=>h.includes('proveedor')||h.includes('nombre'))
      if(provIdx>=0&&cols[provIdx]) proveedores[cols[provIdx]]=(proveedores[cols[provIdx]]||0)+1
    }
  }

  // Construir resumen
  let resumen = `\n📄 ${nombreArchivo}:\n`
  resumen += `  Tipo: ${esIngreso?'INGRESOS':esGasto?'GASTOS':'DATOS'}\n`
  resumen += `  Registros: ${filas}\n`
  resumen += `  Total estimado: $${Math.round(totalMonto).toLocaleString('es-CL')} CLP\n`

  if(esIngreso){
    const topProc=Object.entries(procedimientos).sort((a,b)=>b[1]-a[1]).slice(0,3)
    if(topProc.length>0) resumen+=`  Top procedimientos: ${topProc.map(([k,v])=>`${k.slice(0,30)} (${v}x)`).join(', ')}\n`
  }
  if(esGasto){
    const topProv=Object.entries(proveedores).sort((a,b)=>b[1]-a[1]).slice(0,3)
    if(topProv.length>0) resumen+=`  Principales proveedores: ${topProv.map(([k,v])=>`${k.slice(0,25)} (${v})`).join(', ')}\n`
  }

  // Agregar primeras filas de datos crudos para contexto
  resumen += `  Primeras filas:\n`
  for(let i=1;i<=Math.min(5,lines.length-1);i++) resumen+=`    ${lines[i].slice(0,120)}\n`
...
function parseProveedorRow(line){
  const cols=line.split(',').map(c=>c.trim()); if(cols.length<14) return null
...
function parseProveedoresCSV(csv){
  ...
  for(let i=0;i<Math.min(5,lines.length);i++){const n=lines[i].toLowerCase();if(n.includes('empresa')||n.includes('factura')||n.includes('articulo')||n.startsWith('código')){start=i+1;break}if(lines[i].startsWith('📌'))start=i+1}
  for(let i=start;i<lines.length;i++){const r=parseProveedorRow(lines[i]);if(r?.articulo||r?.empresa) rows.push(r)}
  return rows
}

function parseFacturasCSV(csv){
  const lines=csv.split('\n').map(l=>l.trim()).filter(l=>l&&!l.startsWith('📌')); if(lines.length<2) return []
  const headers=lines[0].split(',').map(h=>h.toLowerCase().trim())
  const iEst=headers.findIndex(h=>h.includes('estado')), iProv=headers.findIndex(h=>h.includes('proveedor')), iMonto=headers.findIndex(h=>h.includes('monto')||h.includes('total')), iDesc=headers.findIndex(h=>h.includes('descripción')||h.includes('descripcion'))
  return lines.slice(1).map(l=>{const c=l.split(',').map(x=>x.trim());return{estado:iEst>=0?c[iEst]||'':'',proveedor:iProv>=0?c[iProv]||'':c[0]||'',monto:iMonto>=0?c[iMonto]||'':'',desc:iDesc>=0?c[iDesc]||'':''}}).filter(r=>r.estado?.toLowerCase().includes('pendiente')||r.estado?.includes('⏳'))
}

// ── IA ────────────────────────────────────────────────────────────
async function askAI(msgs,system){
  const r=await fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${AI_API_KEY}`},body:JSON.stringify({model:AI_MODEL,messages:[{role:'system',content:system},...msgs]})})
...
function Welcome({onLogin}){
  ...
  return(
    <div style={{minHeight:'100vh',background:C.bg,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:24,fontFamily:'inherit'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginBottom:28,maxWidth:620,width:'100%'}}>
        {[['📊','Analiza tus planillas','La IA lee tus archivos de Drive y te dice cómo va tu clínica con números reales.'],['🔍','Busca proveedores','Encuentra insumos, precios y compara quién vende más barato.'],['🔄','Sync automático','Agrega datos en DentaIQ y se guardan en Drive automáticamente.']].map(([i,t,d])=>(
          <div key={t} style={{background:C.white,borderRadius:13,padding:16,border:`1px solid ${C.border}`}}><div style={{fontSize:24,marginBottom:8}}>{i}</div><div style={{fontSize:13,fontWeight:700,color:C.text,marginBottom:4}}>{t}</div><div style={{fontSize:12,color:C.sub,lineHeight:1.5}}>{d}</div></div>
        ))}
      </div>
      <div style={{background:C.white,borderRadius:16,padding:'26px 30px',border:`1px solid ${C.border}`,boxShadow:`0 2px 12px ${C.shadow}`,width:'100%',maxWidth:340,textAlign:'center'}}>
        <div style={{fontSize:13,fontWeight:700,color:C.text,marginBottom:3}}>Empieza gratis</div>
        <div style={{fontSize:12,color:C.sub,marginBottom:16}}>Sesión recordada por 50 min automáticamente</div>
        ...
        <p style={{margin:0,fontSize:10,color:C.light}}>Solo busca en tu carpeta DentaIQ de Drive</p>
      </div>
    </div>
  )
}
...
function Reconnecting({savedUser,onLogin,onLogout}){
  ...
  return(
    <div style={{minHeight:'100vh',background:C.bg,display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'inherit'}}>
      <div style={{background:C.white,borderRadius:16,padding:32,maxWidth:340,width:'100%',textAlign:'center',border:`1px solid ${C.border}`,boxShadow:`0 4px 20px ${C.shadow}`}}>
        <div style={{fontSize:40,marginBottom:12}}>🦷</div>
        <h2 style={{margin:'0 0 6px',fontSize:18,fontWeight:800,color:C.text}}>Bienvenido de vuelta</h2>
        <p style={{fontSize:12,color:C.sub,marginBottom:18}}>Sesión expiró. Un clic para reconectar.</p>
        <div style={{display:'flex',alignItems:'center',gap:8,padding:'8px 11px',background:C.bg,borderRadius:8,marginBottom:16,border:`1px solid ${C.border}`}}>
...
function Onboarding({user,onDone}){
  ...
  const steps=[{icon:'👋',title:`Hola ${user.name.split(' ')[0]}`,desc:'DentaIQ solo trabaja con tu carpeta "DentaIQ" en Drive. Tu sesión se recordará automáticamente.'},{icon:'🔄',title:'Datos reales de Drive',desc:'La IA lee tus planillas de Ingresos y Gastos directamente. Agrega datos en DentaIQ y también se guardan en Drive.'},{icon:'🔍',title:'Buscador de proveedores',desc:'Busca insumos en todos tus archivos. Compara precios entre proveedores.'},{icon:'🤖',title:'IA con tus números reales',desc:'El Consejero IA lee todas tus planillas y te dice exactamente cómo va tu clínica, con cifras reales.'}]
  const s=steps[step]
  return(
...
function Dashboard({user,txs,allFolders,aiSummary,loadingAI}){
  ...
  const margen=ing-gas, pct=ing>0?(margen/ing)*100:0
  const [facturas,setFacturas]=useState([])

  useEffect(()=>{
    if(user.token==='demo') return
    const f=allFolders.find(f=>f.name.toLowerCase().includes('factura'))
    if(!f||!f.files.length) return
    const load=async()=>{try{const csv=await driveReadCSV(user.token,f.files[0].id);setFacturas(parseFacturasCSV(csv))}catch{}}
    load()
  },[allFolders])

  const totalFact=facturas.reduce((s,f)=>{const n=parseFloat(f.monto?.replace(/[$,.\s]/g,'')||'0');return s+(isNaN(n)?0:n)},0)
  ...
  return(
    <div style={{padding:'24px 28px',maxWidth:940}}>
      ...
      <div style={{background:margen>=0?C.tealL:C.redL,borderRadius:16,padding:'16px 20px',marginBottom:16,border:`1px solid ${margen>=0?C.teal+'33':C.red+'33'}`}}>
        <div style={{display:'flex',alignItems:'center',gap:11}}>
          <span style={{fontSize:26}}>{margen>=0?'✅':'⚠️'}</span>
          <div>
            <div style={{fontSize:15,fontWeight:800,color:margen>=0?C.tealD:C.red}}>{txs.length===0?'Conecta Drive para ver tu situación financiera':'Tu clínica está '+(margen>=0?'ganando dinero':'en pérdida este mes')}</div>
            {txs.length>0&&<div style={{fontSize:12,color:margen>=0?C.teal:C.red}}>Margen: {pct.toFixed(1)}% — {pct>40?'excelente 🌟':pct>25?'bien':'necesita atención'}</div>}
          </div>
        </div>
      </div>
      {txs.length>0&&(
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:11,marginBottom:16}}>
          {[{label:'Lo que entró',value:clp(ing),icon:'📈',c:C.green,bg:C.greenL},{label:'Lo que salió',value:clp(gas),icon:'📉',c:C.red,bg:C.redL},{label:'Lo que queda',value:clp(margen),icon:margen>=0?'💰':'🚨',c:margen>=0?C.teal:C.red,bg:margen>=0?C.tealL:C.redL}].map(k=>(
            <div key={k.label} style={{background:k.bg,borderRadius:12,padding:'14px 16px',border:`1px solid ${k.c}33`}}>
              <div style={{fontSize:22,marginBottom:5}}>{k.icon}</div><div style={{fontSize:11,color:C.sub,marginBottom:2}}>{k.label}</div><div style={{fontSize:22,fontWeight:800,color:k.c}}>{k.value}</div>
            </div>
          ))}
        </div>
      )}
      <div style={{display:'grid',gridTemplateColumns:'2fr 1fr',gap:12,marginBottom:16}}>
        <div style={{background:C.white,borderRadius:14,padding:'16px 20px',border:`1px solid ${C.border}`,boxShadow:`0 1px 6px ${C.shadow}`}}>
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
            ...
            <div><div style={{fontSize:13,fontWeight:700,color:C.text}}>Consejero IA</div><div style={{fontSize:10,color:C.sub}}>Análisis de planillas Drive</div></div>
            {loadingAI&&<div style={{marginLeft:'auto',fontSize:10,color:C.sub}}>Analizando...</div>}
          </div>
          {aiSummary?<p style={{margin:0,fontSize:13,color:C.text,lineHeight:1.7,whiteSpace:'pre-wrap'}}>{aiSummary}</p>:<p style={{margin:0,fontSize:12,color:C.sub,fontStyle:'italic'}}>Abre el Consejero IA y pregúntale sobre tu negocio.</p>}
        </div>
        <div style={{background:facturas.length>0?C.orangeL:C.white,borderRadius:14,padding:'16px 18px',border:`1px solid ${facturas.length>0?C.orange+'44':C.border}`,boxShadow:`0 1px 6px ${C.shadow}`}}>
          <div style={{display:'flex',alignItems:'center',gap:7,marginBottom:10}}>
            <div style={{width:30,height:30,borderRadius:8,background:facturas.length>0?C.orange:C.light,display:'flex',alignItems:'center',justifyContent:'center',fontSize:14}}>🧾</div>
            <div><div style={{fontSize:12,fontWeight:700,color:C.text}}>Facturas por Pagar</div><div style={{fontSize:10,color:C.sub}}>{facturas.length>0?`${facturas.length} pendiente${facturas.length>1?'s':''}`:user.token==='demo'?'Demo':'Sin datos'}</div></div>
          </div>
          {facturas.length>0?(
            <>
              <div style={{fontSize:18,fontWeight:800,color:C.orange,marginBottom:7}}>Total: {facturas[0].monto||'—'}</div>
              {facturas.slice(0,3).map((f,i)=><div key={i} style={{fontSize:11,color:C.text,display:'flex',justifyContent:'space-between',marginBottom:3}}><span style={{overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',maxWidth:'65%'}}>{f.proveedor||f.desc||'—'}</span><span style={{fontWeight:700,color:C.orange,flexShrink:0}}>{f.monto||'—'}</span></div>)}
              {facturas.length>3&&<div style={{fontSize:10,color:C.sub}}>+{facturas.length-3} más...</div>}
            </>
          ):<div style={{fontSize:12,color:C.sub}}>{user.token==='demo'?'Conecta Google para ver facturas':'Agrega archivos en carpeta "Facturas por Pagar"'}</div>}
        </div>
      </div>
      <div style={{background:C.white,borderRadius:14,padding:'14px 18px',border:`1px solid ${C.border}`,boxShadow:`0 1px 6px ${C.shadow}`}}>
        <div style={{fontSize:12,fontWeight:700,color:C.text,marginBottom:11}}>☁️ Carpetas en DentaIQ Drive</div>
        <div style={{display:'grid',gridTemplateColumns:`repeat(${Math.min(allFolders.length||2,5)},1fr)`,gap:8}}>
          {allFolders.map(f=><div key={f.id} style={{textAlign:'center',padding:'10px 7px',background:C.bg,borderRadius:10,border:`1px solid ${C.border}`}}><div style={{fontSize:20,marginBottom:3}}>{f.emoji}</div><div style={{fontSize:10,color:C.text,fontWeight:500,marginBottom:2}}>{f.name}</div><div style={{fontSize:10,color:f.files.length>0?C.teal:C.light}}>{f.files.length>0?`${f.files.length} archivos`:'Vacía'}</div></div>)}
        </div>
      </div>
    </div>
  )
...
function MisNumeros({txs,setTxs,user,folderIds}){
  ...
  const ing=txs.filter(t=>t.tipo==='I').reduce((s,t)=>s+t.monto,0), gas=txs.filter(t=>t.tipo==='G').reduce((s,t)=>s+t.monto,0)
  ...
  const guardar=async()=>{
    ...
    if(user.token!=='demo'&&folderIds){
      try{const fId=form.tipo==='I'?folderIds.ingresos:folderIds.gastos; if(fId){const sId=await findOrCreateMonthSheet(user.token,fId,form.tipo==='I'?'Ingresos':'Gastos'); if(sId){const vals=form.tipo==='I'?[fecha,form.desc,form.cat,parseInt(form.monto),form.metodo,'DentaIQ']:[fecha,form.desc,form.cat,parseInt(form.monto),'','DentaIQ']; await appendRowToSheet(user.token,sId,vals); setSyncMsg('✅ Guardado en Drive')}}}
  ...
  return(
    <div style={{padding:'24px 28px',maxWidth:860}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:16}}>
        <div><h1 style={{margin:0,fontSize:21,fontWeight:800,color:C.text}}>Mis Números</h1><p style={{margin:'3px 0 0',fontSize:12,color:C.sub}}>Sincroniza automáticamente con Google Drive</p></div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10,marginBottom:16}}>
        {[['Lo que entró',ing,C.green,C.greenL,'📈'],['Lo que salió',gas,C.red,C.redL,'📉'],['Balance',ing-gas,ing-gas>=0?C.teal:C.red,ing-gas>=0?C.tealL:C.redL,ing-gas>=0?'✅':'⚠️']].map(([l,v,c,bg,ico])=>(
          <div key={l} style={{background:bg,borderRadius:12,padding:'13px 15px',border:`1px solid ${c}33`}}><div style={{fontSize:18,marginBottom:5}}>{ico}</div><div style={{fontSize:11,color:C.sub,marginBottom:2}}>{l}</div><div style={{fontSize:20,fontWeight:800,color:c}}>{clp(v)}</div></div>
        ))}
      </div>
      {txs.length>0&&(
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:16}}>
          {[['📉 Gastos por categoría',catG,C.red,maxG],['📈 Ingresos por categoría',catI,C.green,maxI]].map(([title,data,color,maxVal])=>(
            <div key={title} style={{background:C.white,borderRadius:12,padding:'14px',border:`1px solid ${C.border}`}}>
              ...
              {data.length===0?<div style={{fontSize:12,color:C.sub}}>Sin datos</div>:data.map(({cat,total})=>(
                <div key={cat} style={{marginBottom:7}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:2}}><span style={{fontSize:11,color:C.text}}>{cat}</span><span style={{fontSize:11,fontWeight:700,color:color}}>{clp(total)}</span></div><div style={{height:4,background:C.bg,borderRadius:3}}><div style={{height:'100%',width:`${(total/maxVal)*100}%`,background:color,borderRadius:3}}/></div></div>
              ))}
            </div>
          ))}
        </div>
      ...
      {open&&(
        <div style={{background:C.white,borderRadius:13,padding:18,marginBottom:12,border:`1px solid ${C.border}`,boxShadow:`0 2px 8px ${C.shadow}`}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'2fr 1fr auto',gap:9,alignItems:'flex-end'}}>
            ...
            <div style={{display:'flex',gap:6}}><button onClick={guardar} style={{background:C.teal,color:'#fff',border:'none',borderRadius:7,padding:'8px 14px',fontSize:12,fontWeight:700,cursor:'pointer',whiteSpace:'nowrap'}}>💾 Guardar</button><button onClick={()=>setOpen(false)} style={{background:C.bg,color:C.sub,border:`1px solid ${C.border}`,borderRadius:7,padding:'8px 11px',fontSize:12,cursor:'pointer'}}>✕</button></div>
          </div>
          {user.token!=='demo'&&<p style={{margin:'7px 0 0',fontSize:10,color:C.sub}}>💡 Se sincronizará con tu planilla de Drive automáticamente</p>}
        </div>
      )}
      {txs.length===0?<div style={{background:C.white,borderRadius:12,padding:'28px',textAlign:'center',border:`2px dashed ${C.border}`}}><div style={{fontSize:32,marginBottom:9}}>📋</div><div style={{fontSize:13,fontWeight:600,color:C.text,marginBottom:5}}>Nada registrado aún</div><div style={{fontSize:12,color:C.sub}}>Agrega un ingreso o gasto — se sincronizará con Drive</div></div>
        :<div style={{background:C.white,borderRadius:12,border:`1px solid ${C.border}`,overflow:'hidden',boxShadow:`0 1px 6px ${C.shadow}`}}>
          <div style={{padding:'9px 15px',borderBottom:`1px solid ${C.border}`,fontSize:12,fontWeight:700,color:C.text}}>Últimas transacciones</div>
          {txs.slice(0,20).map((tx,i)=><div key={tx.id} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 15px',borderBottom:i<Math.min(txs.length,20)-1?`1px solid ${C.border}`:'none'}}><div style={{width:34,height:34,borderRadius:8,background:tx.tipo==='I'?C.greenL:C.redL,display:'flex',alignItems:'center',justifyContent:'center',fontSize:15,flexShrink:0}}>{tx.tipo==='I'?'📈':'📉'}</div><div style={{flex:1,minWidth:0}}><div style={{fontSize:12,color:C.text,fontWeight:500,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{tx.desc}</div><div style={{fontSize:10,color:C.sub,marginTop:1}}>{tx.cat} · {tx.fecha}</div></div><div style={{fontSize:13,fontWeight:800,color:tx.tipo==='I'?C.green:C.red,flexShrink:0}}>{tx.tipo==='I'?'+':'−'}{clp(tx.monto)}</div></div>)}
        </div>
      }
    </div>
  )
...
// ── CONSEJERO IA — FIX: useRef para driveCtx ─────────────────────
function Consejero({user,txs,allFolders}){
  const CHAT_KEY=`dentaiq_chat_${user.email}`
  const ing=txs.filter(t=>t.tipo==='I').reduce((s,t)=>s+t.monto,0)
  const gas=txs.filter(t=>t.tipo==='G').reduce((s,t)=>s+t.monto,0)

  // ✅ FIX: useRef para evitar el problema de closure
  // El estado es solo para mostrar el status, la ref tiene el valor real
  const driveCtxRef=useRef('')
  const driveResumsRef=useRef([]) // resúmenes procesados
  const [driveStatus,setDriveStatus]=useState('idle')
  const [driveSummaryText,setDriveSummaryText]=useState('')

  const [msgs,setMsgs]=useState(()=>{try{const s=localStorage.getItem(CHAT_KEY);return s?JSON.parse(s):[{role:'assistant',content:`Hola ${user.name.split(' ')[0]} 👋 Cargando tus planillas de Drive...`}]}catch{return[{role:'assistant',content:`Hola 👋 ¿En qué te ayudo?`}]}})
  ...
  // Cargar Drive cuando allFolders tenga archivos
  useEffect(()=>{
    ...
    const load=async()=>{
      ...
      let fullCtx='', resums=[], totalIng=0, totalGas=0

      for(const folder of allFolders){
        for(const file of folder.files.slice(0,2)){
          if(!file.mimeType?.includes('spreadsheet')) continue
          try{
            const csv=await driveReadCSV(user.token,file.id)
            const analisis=resumirCSVParaIA(csv,file.name)
            if(analisis){
              resums.push(analisis)
              fullCtx+=analisis.resumen
              if(analisis.esIngreso) totalIng+=analisis.totalMonto
              if(analisis.esGasto) totalGas+=analisis.totalMonto
            }
          }catch(e){console.warn('No se pudo leer',file.name,e.message)}
        }
      }

      // Guardar en refs (no en state) para evitar closure stale
      driveCtxRef.current=fullCtx
      driveResumsRef.current=resums

      const summary=fullCtx
        ?`📊 Resumen de Drive:\n• Total ingresos Drive: $${Math.round(totalIng).toLocaleString('es-CL')} CLP\n• Total gastos Drive: $${Math.round(totalGas).toLocaleString('es-CL')} CLP\n• Balance: $${Math.round(totalIng-totalGas).toLocaleString('es-CL')} CLP\n• Archivos analizados: ${resums.length}`
        :''

      setDriveSummaryText(summary)
      setDriveStatus(fullCtx?'ready':'empty')

      setMsgs(prev=>{
        const last=prev[prev.length-1]
        if(last?.role==='assistant'&&(last.content.includes('Cargando')||last.content.includes('leyendo'))){
          const msg=fullCtx
            ?`Hola ${user.name.split(' ')[0]} 👋 Leí ${resums.length} planillas de Drive. ${summary}\n\n¿Qué quieres saber sobre tus finanzas?`
            :`Hola ${user.name.split(' ')[0]} 👋 No encontré planillas con datos en Drive todavía. Sube tus archivos a las carpetas de DentaIQ.`
  ...
  // ✅ FIX: system se construye DENTRO de send() usando la ref
  const send=async(texto)=>{
    const t=(texto||inp).trim(); if(!t||loading) return
    setInp('')

    // Construir system con el valor ACTUAL de la ref (siempre fresco)
    const ctx=driveCtxRef.current
    const system=`Eres el consejero financiero de ${user.name}, clínica dental en Chile. Lenguaje simple, como amigo experto sin jerga contable.

DATOS REGISTRADOS EN APP: Ingresos $${ing} CLP, Gastos $${gas} CLP, Balance $${ing-gas} CLP.

${ctx?`DATOS REALES DE PLANILLAS DRIVE (ESTOS SON LOS MÁS IMPORTANTES - ÚSALOS PARA RESPONDER):\n${ctx}\n\nNOTA: Los montos en las planillas pueden estar en USD. Si ves valores como $35.00, $45.00, multiplica por ~930 para obtener CLP aprox.`:'DRIVE: Sin planillas cargadas todavía.'}

INSTRUCCIONES:
- Si hay datos de Drive, ÚSALOS para responder con cifras específicas
- Calcula totales sumando las columnas EFECTIVO + TRANSFERENCIA + TARJETA
- Habla como amigo, usa emojis, máx 250 palabras, español chileno
- Da siempre un consejo concreto y accionable`
  ...
  const statusLabel=driveStatus==='loading'?'⏳ Leyendo planillas...':driveStatus==='ready'?`✅ ${driveResumsRef.current.length} planillas cargadas`:driveStatus==='empty'?'📋 Sin datos en Drive':'⏳ Esperando...'
  const SUGS=['¿Estoy ganando suficiente?','¿En qué me gasto más?','Analiza mis ingresos Drive','¿Cuánto gané este mes?','¿Qué puedo mejorar?']

  return(
    <div style={{display:'flex',flexDirection:'column',height:'100vh',maxWidth:800,padding:'0 24px'}}>
      <div style={{padding:'20px 0 12px',display:'flex',justifyContent:'space-between',alignItems:'flex-end'}}>
        <div><h1 style={{margin:0,fontSize:20,fontWeight:800,color:C.text}}>Consejero IA 🤖</h1><p style={{margin:'2px 0 0',fontSize:11,color:C.sub}}>{statusLabel}</p></div>
        <div style={{display:'flex',gap:6}}>
          {driveStatus==='ready'&&<button onClick={()=>setDriveStatus('idle')} style={{background:'transparent',border:`1px solid ${C.border}`,color:C.sub,borderRadius:7,padding:'4px 9px',fontSize:11,cursor:'pointer'}}>↻ Recargar</button>}
...
function Buscador({user,allFolders}){
    ...
    const load=async()=>{
      ...
      for(const folder of allFolders){
        for(const file of folder.files){
          if(!file.mimeType?.includes('spreadsheet')) continue
          try{const csv=await driveReadCSV(user.token,file.id); const parsed=parseProveedoresCSV(csv); if(parsed.length>0) rows.push(...parsed.map(r=>({...r,_carpeta:folder.name,_archivo:file.name})))}
          catch{}
        }
      }
  ...
  const th={padding:'8px 12px',fontSize:11,fontWeight:700,color:C.sub,background:C.bg,borderBottom:`1px solid ${C.border}`,textAlign:'left',whiteSpace:'nowrap'}
  const td={padding:'8px 12px',fontSize:11,color:C.text,borderBottom:`1px solid ${C.border}`,verticalAlign:'middle'}

  return(
    <div style={{padding:'22px 26px',maxWidth:1080}}>
      <div style={{marginBottom:14}}><h1 style={{margin:0,fontSize:20,fontWeight:800,color:C.text}}>Buscador de Insumos y Proveedores 🔍</h1><p style={{margin:'3px 0 0',fontSize:12,color:C.sub}}>Busca en todas las carpetas de DentaIQ · Filtra · Compara precios</p></div>
      <div style={{display:'flex',gap:7,marginBottom:12}}>
        {[['tabla','📋 Ver todos'],['comparar','⚖️ Comparar precios']].map(([m,l])=><button key={m} onClick={()=>setModo(m)} style={{padding:'6px 14px',borderRadius:17,border:`2px solid ${modo===m?C.teal:C.border}`,background:modo===m?C.tealL:'transparent',color:modo===m?C.tealD:C.sub,fontSize:12,fontWeight:modo===m?700:400,cursor:'pointer'}}>{l}</button>)}
      </div>
      ...
      {modo==='tabla'&&<>
        ...
        {loading&&<div style={{textAlign:'center',padding:36,color:C.sub}}>Cargando desde Drive...</div>}
        ...
        {!loading&&loaded&&<>
          ...
          <div style={{background:C.white,borderRadius:12,border:`1px solid ${C.border}`,overflow:'auto',boxShadow:`0 1px 6px ${C.shadow}`}}>
            <table style={{width:'100%',borderCollapse:'collapse',minWidth:900}}>
              ...
              <tbody>
                {filas.length===0?<tr><td colSpan={11} style={{...td,textAlign:'center',padding:26,color:C.sub}}>No se encontraron resultados{filtro?` para "${filtro}"`:''}.</td></tr>
                  :filas.map((r,i)=>(
      ...
      {modo==='comparar'&&<>
        <div style={{background:C.goldL,borderRadius:10,padding:'10px 13px',marginBottom:12,border:`1px solid ${C.gold}33`,fontSize:12,color:C.text}}>💡 Busca un insumo para comparar precios entre proveedores. <strong>Verde = más barato</strong></div>
        <div style={{display:'flex',gap:8,marginBottom:16}}>
          <input value={comparaTerm} onChange={e=>setComparaTerm(e.target.value)} placeholder="Ej: anestesia, guantes, isocaine, acrilico..." style={{flex:1,background:C.white,border:`1.5px solid ${C.teal}`,color:C.text,borderRadius:9,padding:'10px 13px',fontSize:13,fontFamily:'inherit',outline:'none'}}/>
        </div>
        {comparaTerm&&(()=>{
          ...
          return <div style={{display:'flex',flexDirection:'column',gap:12}}>{grupos.map((g,gi)=>(
            <div key={gi} style={{background:C.white,borderRadius:13,border:`1px solid ${C.border}`,overflow:'hidden',boxShadow:`0 1px 6px ${C.shadow}`}}>
              <div style={{padding:'11px 17px',background:C.bg,borderBottom:`1px solid ${C.border}`,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:`repeat(${Math.min(g.proveedores.length,4)},1fr)`}}>
                {g.proveedores.map((p,pi)=>(
                  <div key={pi} style={{padding:'13px 17px',borderRight:pi<g.proveedores.length-1?`1px solid ${C.border}`:'none',background:pi===0?C.greenL:'transparent'}}>
                    ...
                    <div style={{fontSize:19,fontWeight:800,color:pi===0?C.green:pi===g.proveedores.length-1&&pi!==0?C.red:C.text}}>{p.precioRaw}</div>
...
function MiDrive({user,allFolders,loading,onRefresh,onAnalyze}){
  const [sel,setSel]=useState(null),[reading,setReading]=useState(null)
  ...
  const readAndAnalyze=async file=>{
    setReading(file.id)
    try{const csv=await driveReadCSV(user.token,file.id); onAnalyze(csv,file.name)}
  ...
  return(
    <div style={{padding:'22px 26px',maxWidth:860}}>
      <div style={{display:'flex',justifyContent:'space-between',marginBottom:16}}>
        <div><h1 style={{margin:0,fontSize:20,fontWeight:800,color:C.text}}>Mi Drive ☁️</h1><p style={{margin:'3px 0 0',fontSize:12,color:C.sub}}>Solo muestra archivos dentro de la carpeta DentaIQ</p></div>
      ...
      {loading?<div style={{textAlign:'center',padding:36,color:C.sub}}>Cargando...</div>:<>
        ...
        {activeF&&(
          <div style={{background:C.white,borderRadius:12,border:`1px solid ${C.border}`,overflow:'hidden'}}>
            <div style={{padding:'10px 15px',borderBottom:`1px solid ${C.border}`,display:'flex',justifyContent:'space-between'}}><span style={{fontSize:13,fontWeight:700}}>{activeF.emoji} {activeF.name}</span><span style={{fontSize:11,color:C.sub}}>{activeF.files.length} archivos</span></div>
            {activeF.files.length===0?<div style={{padding:'24px',textAlign:'center',color:C.sub}}>Carpeta vacía</div>:activeF.files.map((f,i)=>(
              <div key={f.id} style={{display:'flex',alignItems:'center',gap:9,padding:'9px 15px',borderBottom:i<activeF.files.length-1?`1px solid ${C.border}`:'none'}}>
                <span style={{fontSize:18}}>{f.mimeType?.includes('spreadsheet')?'📊':'📄'}</span>
                <div style={{flex:1,minWidth:0}}><div style={{fontSize:12,color:C.text,fontWeight:500,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{f.name}</div>{f.modifiedTime&&<div style={{fontSize:10,color:C.sub}}>{new Date(f.modifiedTime).toLocaleDateString('es-CL')}</div>}</div>
                {canRead(f.mimeType)&&<button onClick={()=>readAndAnalyze(f)} disabled={reading===f.id} style={{background:C.tealL,color:C.tealD,border:`1px solid ${C.teal}33`,borderRadius:6,padding:'4px 10px',fontSize:11,fontWeight:600,cursor:'pointer'}}>{reading===f.id?'Leyendo...':'🤖 Analizar'}</button>}
              </div>
            ))}
          </div>
        )}
...
export default function App(){
  ...
  const [aiSummary,setAiSummary]=useState(''),[loadingAI,setLoadingAI]=useState(false)
  ...
  const loadDrive=useCallback(async token=>{
    ...
    try{const {allFolders:af,folderIds:fids}=await setupDrive(token);setAllFolders(af);setFolderIds(fids)}
